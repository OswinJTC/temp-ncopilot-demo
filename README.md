    ...
    async def execute_queries(my_params: RequestParams):
        ...
        try:
            for query in my_params.queries: #可能多個 query ，一個個處理
                ...
                interface = DataInterfaceFactory().get_interface(query.interface_type, query_dict, projection, conditions) #分類參數，送到 factory
...
2. **Factory distributes to interfaces:**
    ...
    class DataInterfaceFactory:
        def get_interface(self, interface_type: str, query: Dict, projection: Dict = None, conditions: Optional[Dict] = None): #決定送到哪個 interface
    ...
    class FindVitalsignsInterface(DataInterface):
        def execute(self): #interface 內部找尋資料過程
            ...
            if self.conditions and self.conditions.get("duration"):
                ...
                query["createdDate"] = {"$gte": start_date, "$lte": end_date}  #獲取日期
            ...
            return [doc for doc in cursor if all(f in doc for f in self.projection.keys() if self.projection[f] == 1)]  #回傳
    ```
